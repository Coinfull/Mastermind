...
class Guess
{
   ...
   public int[] reportResult(Guess other_guess)
   {
		...
		/*
		*
		* For debugging purposes so we know what the answer is
		* Remove before submission
		*/
		/*System.out.println(other_guess_ids.get(0));
		System.out.println(other_guess_ids.get(1));
		System.out.println(other_guess_ids.get(2));
		System.out.println(other_guess_ids.get(3));*/
      ...
      //DO THIS
		/*for(int i = 0; i < other_guess_ids.size(); ++i){
			for(int j = 0; j < current_guess_ids.size(); ++j){
				if(other_guess_ids.get(i) == current_guess_ids.get(j)){
					white_count++;
					break;
				}
			}
		...
		for(int i = 0; i < current_guess_ids.size(); ++i){
			for(int j = 0; j < other_guess_ids.size(); ++j){
				if(current_guess_ids.get(i) == other_guess_ids.get(j)){
					white_count++;
					current_guess_ids.remove(i);
					other_guess_ids.remove(j);
					i--;
					j--;
					break;
				}
